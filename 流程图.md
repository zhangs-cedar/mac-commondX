...
flowchart TD
    ...
    SmartMenu -->|"选择 KIMI AI工具"| KimiApi["Kimi API 处理流程"]
```
...
- **Kimi API 处理流程**：由智能操作菜单触发，处理文件/图片内容，支持翻译、解释、分析等
...
**职责**：由智能操作菜单触发，处理文件/图片内容，调用 AI API 进行翻译、解释、分析等操作

**输入**：用户从智能操作菜单选择「KIMI AI工具」（选中文件后 ⌘+X 选相同文件 → 菜单 → KIMI AI工具）  
**输出**：AI 处理结果（翻译、解释、总结、分析）

```mermaid
flowchart TD
    StartNode["用户从智能菜单选择 KIMI AI工具"] --> GetFiles["获取选中文件路径"]
    GetFiles --> CheckFiles{"是否有文件"}
    
    CheckFiles -->|否| EndNode["结束"]
    CheckFiles -->|是| DetectType["检测内容类型"]
    
    DetectType --> ContentType{"内容类型"}
    ContentType -->|文本| GetText["读取文本内容"]
    ContentType -->|文件| ReadFile["读取文件内容"]
...
- **配置选项**：
  ...
  - **Kimi API 配置**：AI 功能配置，通过"输入 KIMI API Key"菜单项或直接编辑配置文件设置 `kimi_api.api_key` 即可启用
    ...
    - 配置格式：
      ...
      ```
    - 功能说明：
      - 仅通过智能操作菜单触发（⌘+X 选相同文件 → 菜单 → KIMI AI工具）
      - 自动检测内容类型（文本/文件/图片）
      - 支持操作：翻译、解释、总结、分析
