...
```mermaid
flowchart TD
    Start[调用 cut_manager.cut] --> GetSelection[获取 Finder 选中文件]
...
```mermaid
flowchart TD
    Start[执行剪切操作] --> UpdateCutFiles[更新 cut_files = 当前选择]
...
```mermaid
flowchart TD
    Start[触发智能操作菜单] --> ShowMenu[显示文件智能操作菜单]
...
```mermaid
flowchart TD
    Start[执行操作] --> CheckOperation{操作类型}
...
```mermaid
flowchart TD
    Start[用户按下 ⌘+C] --> OnCopy[on_copy 回调函数]
