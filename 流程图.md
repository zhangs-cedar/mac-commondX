...
flowchart TD
    ...
    UserChoice -->|"PDF编辑工具"| PdfEditor["打开 PDF24 工具网页"]
...
- **操作选项**：
  ...
  - **PDF 编辑工具**：打开 PDF24 工具网页（https://tools.pdf24.org/zh），提供免费的在线 PDF 操作工具
...
- **PDF 编辑工具**：使用 `open` 命令在浏览器中打开 PDF24 工具网页，检测 PDF 文件并提示，通知成功
...
状态栏图标菜单
  ...
  │           │   ├── ☐ 文本文件 (复选框，可点击)
  │           │   ├── ☐ 代码文件 (复选框，可点击)
  │           │   ├── ☑ PDF 文件 (复选框，可点击，默认仅此类型)
  │           │   └── ☐ 其他 (复选框，可点击)
  │           └── ↑ 上移 (可点击，调整顺序)
  │       ├── ────────────────
  │       ├── 🔑 输入 API Key (点击后显示简洁输入框)
...
- **配置选项**：
  ...
  - **输入 API Key**：点击后显示简洁输入框，输入 Kimi API Key 并保存到配置文件
    - 输入框：请输入 Kimi API Key
    - 保存成功：通知显示"✅ 保存成功，Kimi API Key 已保存（长度: N）"
    - 保存失败：通知显示"❌ 保存失败，无法保存 API Key: 错误信息"
  ...
  - **Kimi API 配置**：AI 功能配置，通过"输入 API Key"菜单项或直接编辑配置文件设置 `kimi_api.api_key` 即可启用
...
- **操作处理**: `src/status_bar.py` - 各 `smart*_()` 方法（处理具体的智能操作）
  ...
  - `smartPdfEditor_()`: PDF 编辑工具操作（使用 `open` 命令在浏览器中打开 PDF24 工具网页）
...
- **PDF 编辑插件**: `src/plugins/pdf_editor_plugin.py` - PDF 在线工具集成
  - `execute()`: 打开 PDF24 工具网页（https://tools.pdf24.org/zh），提供免费的在线 PDF 操作工具
  - 支持检测 PDF 文件并提示用户
