...
flowchart TD
    ...
    UpdateLastSame --> ReturnShowDialog["返回 (True, True)<br/>触发智能操作弹窗"]
    ...
    ReturnShowDialog --> CloseOldDialog{是否有旧弹窗?}
    CloseOldDialog -->|是| StopModal["关闭旧弹窗<br/>(NSApp.stopModal)"]
...
- 比较结果决定是执行剪切还是显示智能操作弹窗

### 2. 处理规则
- **选择与上次不同**：执行剪切，更新 `cut_files` 和 `last_selection`
- **选择与上次相同**：触发智能操作弹窗，更新 `last_selection`
