...
class CutManager:
    ...
    def cut(self) -> tuple:
        ...
        import time
        print(f"[DEBUG] [CutManager] 调试日志 - sessionId=debug-session, runId=run2, hypothesisId=F, location=cut_manager.py:cut, message=执行剪切操作，更新 cut_files, data={{cut_files_count={len(valid)}, cut_files={valid}, timestamp={time.time()}}}, timestamp={int(time.time() * 1000)}")
        self._notify()
